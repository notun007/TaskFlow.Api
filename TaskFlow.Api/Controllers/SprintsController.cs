...
public sealed record SprintItem(Guid Id, string Name, string? Goal, Guid ProjectId, string Status, DateOnly? StartDate, DateOnly? EndDate, DateTimeOffset? StartedAt, DateTimeOffset? CompletedAt, IReadOnlyList<SprintTaskItem> Tasks);
...
public sealed class SprintsController(IApplicationDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<BacklogDetails>> Backlog(Guid projectId, CancellationToken cancellationToken)
    {
        ...
        var sprints = await db.Sprints.AsNoTracking().Where(x => x.ProjectId == projectId && !x.IsDeleted)
            .OrderBy(x => x.Status == SprintStatus.Active ? 0 : x.Status == SprintStatus.Planned ? 1 : 2).ThenBy(x => x.StartDate).ThenBy(x => x.Name)
            .Select(x => new SprintItem(x.Id, x.Name, x.Goal, x.ProjectId, x.Status.ToString(), x.StartDate, x.EndDate, x.StartedAt, x.CompletedAt,
